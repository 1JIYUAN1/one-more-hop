...
export function checkpoint(step){return checkpointAt(step)}
export function ticketProgress(progress,perfect,tickets){const next=perfect?progress+1:progress;if(tickets>0||next<RULES.ticketPerfects)return {progress:next,tickets,earned:false};return {progress:0,tickets:1,earned:true};}
export function badgeTitle(medals){if(medals>=9)return '月球领航员';if(medals>=5)return '小火箭驾驶员';if(medals>=3)return '纸飞机收集家';return '起跳练习生';}
